...
*/
function bosa_veterinary_slickNavHeight (){
...
*/
function bosa_veterinary_wpMenuAccordion( selector ){
...
function bosa_veterinary_fixed_header(){
  $notificationHight = $( '.notification-bar' ).height();
  $logo_selector = document.getElementById( 'headerLogo' );
  var width = $( window ).width();

  if ( $logo_selector && BOSAVETERINARY.fixed_nav && BOSAVETERINARY.fixed_header_logo ) { 
    if ( $mastheadHeight < $(window).scrollTop()){
      if ( BOSAVETERINARY.separate_logo == '' ){
        if ( BOSAVETERINARY.the_custom_logo !== '' ){
          $logo_selector.src = BOSAVETERINARY.the_custom_logo;
        }
      }else{
        $( '.site-header .site-branding img' ).css( 'display' , 'block' );
        if( !BOSAVETERINARY.mobile_fixed_nav_off || width >= 782 ){
          $logo_selector.src = BOSAVETERINARY.separate_logo;
        }
      }
    }else{
      if ( BOSAVETERINARY.header_two_logo !== '' && ( BOSAVETERINARY.is_front_page || BOSAVETERINARY.overlay_post || BOSAVETERINARY.overlay_page ) && BOSAVETERINARY.is_header_two ){
         $logo_selector.src = BOSAVETERINARY.header_two_logo;
      }else if ( BOSAVETERINARY.the_custom_logo !== '' ) {
          $logo_selector.src = BOSAVETERINARY.the_custom_logo;
      }else if ( BOSAVETERINARY.separate_logo !== '' ){
        $( '.site-header .site-branding img' ).css( 'display' , 'none' );
      }
    }
  }
  if ( $mastheadHeight > $( window ).scrollTop() || $( window ).scrollTop() == 0 ) {
      if ( BOSAVETERINARY.fixed_nav && $( '#masthead.site-header' ).hasClass( 'sticky-header' )){
          $( '#masthead.site-header' ).removeClass( 'sticky-header' );
          // Fixed header in admin bar
          if( BOSAVETERINARY.is_admin_bar_showing && width >= 782 ){
            $( '.fixed-header' ).css( 'marginTop', 0 );
          }
          if( BOSAVETERINARY.is_admin_bar_showing && width <= 781 ){
            $( '.fixed-header' ).css( 'marginTop', 0 );
          }
      }
  }else if ( BOSAVETERINARY.fixed_nav && !$( '#masthead.site-header' ).hasClass( 'sticky-header' )){
    if( !BOSAVETERINARY.mobile_fixed_nav_off || width >= 782 ){
      ...
      if( BOSAVETERINARY.is_admin_bar_showing && width <= 600 ){
        $( '.fixed-header' ).css( 'marginTop', 0 );
      }
      if( BOSAVETERINARY.mobile_fixed_nav_off && width <= 781 ){
        $( '.fixed-header' ).css( 'marginTop', 0 );
    }
  }
}
...
*/
function bosa_veterinary_header_two_postion() {
  var width = $( window ).width();

  if ( BOSAVETERINARY.is_header_two ) {
    if( BOSAVETERINARY.is_admin_bar_showing && width >= 782 ){
...
jQuery( document ).ready( function() {
  bosa_veterinary_slickNavHeight();
  bosa_veterinary_wpMenuAccordion( '#offcanvas-menu' );
  bosa_veterinary_header_two_postion();
  ...
  */
  $( '.header-image-slider' ).slick({
      ...
      fade: BOSAVETERINARY.header_image_slider.fade,
      speed: parseInt( BOSAVETERINARY.header_image_slider.fadeControl ),
      cssEase: 'linear',
      autoplay: BOSAVETERINARY.header_image_slider.autoplay,
      autoplaySpeed: BOSAVETERINARY.header_image_slider.autoplaySpeed,
      infinite: true,
  ...
  */
  $( '#primary-menu' ).slicknav({
      ...
      label: BOSAVETERINARY.responsive_header_menu_text,
  ...
  */
  $( '.main-slider' ).slick({
      ...
      arrows: true,
      adaptiveHeight: false,
      fade: BOSAVETERINARY.main_slider.fade,
      speed: parseInt( BOSAVETERINARY.main_slider.fadeControl ),
      cssEase: 'linear',
      autoplay: BOSAVETERINARY.main_slider.autoplay,
      autoplaySpeed: BOSAVETERINARY.main_slider.autoplaySpeed,
      infinite: true,
  ...
  $( '.highlight-post-slider' ).slick({
      arrows: true,
      dots: true,
      slidesToShow: BOSAVETERINARY.home_highlight_posts.slidesToShow,
      slidesToScroll: 1,
      adaptiveHeight: false,
      autoplay: BOSAVETERINARY.home_highlight_posts.autoplay,
      autoplaySpeed: BOSAVETERINARY.home_highlight_posts.autoplaySpeed,
      infinite: false,
      rows: 0,
  ...
  * Sticky sidebar
  */
  if( BOSAVETERINARY.sticky_sidebar ){
...
jQuery( window ).on( 'load', function(){
  ...
  * Back to top
  */
  if( BOSAVETERINARY.enable_scroll_top == true && $(window).scrollTop() > 200 ){
...
jQuery( window ).on( 'scroll', function(){
  ...
  * Back to top
  */
  if( BOSAVETERINARY.disable_scroll_top == false && $(window).scrollTop() > 200 ){
    $( '#back-to-top' ).fadeIn( 200 );
  } else{
    $( '#back-to-top' ).fadeOut( 200 );
  }
